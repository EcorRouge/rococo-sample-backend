...
def main():
    ...
    if state:
        ...
        raise ADWError(
            message=f"No state found for ADW ID: {adw_id}. Run adw_plan_iso.py or adw_patch_iso.py first to create the worktree and state",
    ...
    if not valid:
        raise ADWError(
            message=f"Worktree validation failed: {error}. Run adw_plan_iso.py or adw_patch_iso.py first",
            issue_number=issue_number,
            adw_id=adw_id,
            agent_name="ops",
        )
    ...
    # Get port information for display (backend-only)
    ...
    # Create commit message (non-critical, log but don't fail)
    ...
    if error:
        ...
        if not success:
            logger.warning(f"Error committing documentation: {error}")
