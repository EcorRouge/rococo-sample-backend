...
class ADWState:
    ...
    def update(self, **kwargs):
        ...
        core_fields = {"adw_id", "issue_number", "branch_name", "plan_file", "issue_class", "worktree_path", "backend_port", "model_set", "all_adws"}
    ...
    def save(self, workflow_step: Optional[str] = None) -> None:
        ...
        with open(state_path, "w") as f:
            json.dump(state_data.as_dict(), f, indent=2)
    ...
    def load(
        ...
        try:
            ...
            state_data = ADWStateData.from_dict(data)

            # Create ADWState instance
            state = cls(state_data.adw_id)
            state.data = state_data.as_dict()

            if logger:
                logger.info(f"🔍 Found existing state from {state_path}")
                logger.info(f"State: {json.dumps(state_data.as_dict(), indent=2)}")
