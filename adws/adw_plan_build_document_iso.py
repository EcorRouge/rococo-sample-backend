...
Usage: uv run adws/adw_plan_build_document_iso.py <issue-number> [adw-id]
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: uv run adws/adw_plan_build_document_iso.py <issue-number> [adw-id]")
    ...
    if document.returncode != 0:
        print(f"❌ Isolated documentation phase failed with exit code {document.returncode}")
        sys.exit(document.returncode)
