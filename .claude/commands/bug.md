...
- If you need a new library, use `uv add` (in flask directory) or add to pyproject.toml and be sure to report it in the `Notes` section of the `Plan Format`.
...
- `common/**` - Contains shared backend modules (models, repositories, services, helpers, utils).
- `flask/**` - Contains the Flask application code.
- `tests/**` - Contains the test suite.
- `adws/**` - Contains the AI Developer Workflow (ADW) scripts.
- `services/**` - Contains service configurations (PostgreSQL, RabbitMQ, Email Transmitter).
- `docker-compose.yml` - Contains Docker service definitions.
...
- `pytest tests/ -v` - Run tests to validate the bug is fixed with zero regressions

## Notes
<optionally list any additional notes or context that are relevant to the bug that will be helpful to the developer>## Bug
