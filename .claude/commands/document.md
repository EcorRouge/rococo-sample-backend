...
- Filename format: `feature-{adw_id}-{descriptive-name}.md`
  - Replace `{descriptive-name}` with a short feature name (e.g., "user-auth", "api-endpoint", "service-integration")
- Follow the Documentation Format below
- Focus on:
  - What was built (based on git diff)
  - How it works (technical implementation)
  - How to use it (API/user perspective)
...
<Any additional context, limitations, or future considerations>## Conditional Docs Entry Format
...
- app_docs/<your_documentation_file>.md
  - Conditions:
    ...
    - When troubleshooting <specific issues>## Report
